...
products2.reset_index(inplace=True, drop=True)
print(products2)

# 문제 08) 상품명 열을 인덱스로 설정한 products3 DataFrame을 생성하세요.
products3 = products2.set_index('상품명')

# 문제 09) products3 DataFrame에서 인덱스 값 노트북을 고성능노트북으로 변경하세요.
# idx = list(products3.index)[1:]
# idx.insert(0,'고성능노트북')
# products3.index = idx
...
products3.rename({'노트북':'고성능노트북'}, axis=0, inplace=True)
print(products3)

# 문제 10) products3 DataFrame의 인덱스를 오름차순으로 정렬하세요.
products3.sort_index(inplace=True)
print(products3)

# 문제 11) products3 DataFrame의 인덱스를 내림차순으로 정렬하세요.
products3.sort_index(inplace=True, ascending=False)
print(products3)

# 문제 12) products3 DataFrame의 인덱스를 초기화하여 0, 1, 2, 3, 4 형태의
# 정수 인덱스로 변경하세요.
products3.reset_index(inplace=True, names='상품명')
print(products3)

# 문제 15) 다음 조건을 모두 수행하세요.
# → products DataFrame에서 상품코드를 인덱스로 설정하세요.
products.set_index('상품코드', inplace=True)
print(products)
...
products.loc['P004', '가격'] = 170000
print(products)

# → 인덱스를 다시 일반 컬럼으로 초기화하세요.
products.reset_index(inplace=True, names='상품코드')
