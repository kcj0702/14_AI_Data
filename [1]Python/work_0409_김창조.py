...
for file in files:
    if (file[file.find(".")+1:] == "h") or (file[file.find(".")+1:] == "c"):
        print(file)
...
for file in files:
    if (file[:file.find(".")].count("e") >= 2) and (file[:file.find(".")].count("f") >= 1):
        print(file)
...
for dan in range(3,10,4):
    print(f"{" " * 4}{dan}{" " * 16}{dan + 2}")
    for n in range(1,10):
        print(f"{dan} * {n} = {dan * n:02d}       {dan + 2} * {n} = {(dan + 2) * n:02d}")
    print()
...
def cal(a:int, b:int, op:str):
    ...
    if op == "+":
        ...
        if b != 0:
            ...
            print("두 번째 정수는 1이상의 숫자를 입력해주세요.")
    else:
...
print("가나*마바사*자***파하".find("*"))

idx = "가나*마바사*자***파하".find("*")
for _ in range(2):
    idx = "가나*마바사*자***파하".find("*", idx + 1)
print(idx)
...
word = input("좋아하는 단어를 입력해주세요.")
print(word.upper())

for spell in set(word):
    print(f"{spell} - {word.count(spell)}개", end="     ")
...
bunny = "산토끼 토끼야 어디를 가느냐 폴짝 폴짝 뛰면서 어디를 가느냐"
ynnub = bunny.split(" ")
for i in ynnub:
    print(i[::-1], end=" ")
...
li1 = set([1,1,5,2,6,9,2])
li2 = set([5,9,1,3,4,2,8,7,1,2,5])
li3 = list(li1.union(li2))
print(li3)
...
message = list(input("메시지를 입력해주세요."))
for i in message:
    print(hex(ord(i)), end="")
print("\n사실 안배워서 AI로 물어봤습니다.")
